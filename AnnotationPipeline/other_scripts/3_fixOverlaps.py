...
# Input file paths
gff_file = "2_Galeolaria_caespitosa_functional_productFix.gff3"

# Output file path
output_gff = "3_Galeolaria_caespitosa_functional_fixOverlaps.gff3"
output_db = "Galeolaria_caespitosa_fixOverlaps.gffdb"
    ...
    for feat in db.all_features():
        ...
        if feat.featuretype == 'gene':
            ...
            if feat.id in genes_to_remove:
                ...
                new_id = f"galc_g{gene_num}"
                new_locus_tag = f'ABL822_' + str(int(gene_num)).zfill(5)
            ...
            if gene_parent in genes_to_remove:
                ...
                feat.attributes['ID'][0] = re.sub("galc_g\d+", new_id, feat.attributes['ID'][0])
                feat.attributes['Parent'][0] = re.sub("galc_g\d+", new_id, feat.attributes['Parent'][0])
                feat.attributes['transcript_id'][0] = re.sub("ABL822_\d{5}", new_locus_tag, feat.attributes['transcript_id'][0])
                fout.write(f"{str(feat)}\n")
            ...
            if gene_parent in genes_to_remove:
                ...
                feat.attributes['ID'][0] = re.sub("galc_g\d+", new_id, feat.attributes['ID'][0])
                feat.attributes['Parent'][0] = re.sub("galc_g\d+", new_id, feat.attributes['Parent'][0])
                feat.attributes['protein_id'][0] = re.sub("ABL822_\d{5}", new_locus_tag, feat.attributes['protein_id'][0])
                fout.write(f"{str(feat)}\n")
